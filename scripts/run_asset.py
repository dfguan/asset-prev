...
def assess_bn_core(p):
    [man, mem_lim, core_lim, queue, ref, fn, out_dir, out_fn, bin_dir, spid] = p
    ...
    if not rtn:
        ...
        if not rtn:
            ...
            if not rtn:
                ...
                rtn = man.start([j], True) 
    return rtn
def assess_bn(man, ref, fofn_list, core_lim, mem_lim, queue, out_dir, bin_dir, spid, skip_bn):
    if skip_bn == 1:
        return 
    procs = []
    out_fns = []
    ...
    for i in range(len(fofn_list)):
        ...
        out_fns.append(out_fn)
        params.append([man, mem_lim, core_lim, queue, ref, fofn_list[i], out_dir, out_fn, bin_dir, spid])
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        p.join()
        # gen punch list
        p = Process(target=punchlist, args=(man, out_dir, bin_dir, spid))
